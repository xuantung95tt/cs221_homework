...
def formula1b():
    ...
    # BEGIN_YOUR_CODE (around 1 line of code expected)
    return And(Implies(Or(Rain, Sprinklers),Wet), Implies(Not(Or(Rain, Sprinklers)), Not(Wet)))
    # END_YOUR_CODE
...
def formula2a():
    ...
    # BEGIN_YOUR_CODE (around 1 line of code expected)
    #return Forall("$x", Exists("$y", Mother(Person('$x'),Person('$y'))))
    #return Forall("$x", Exists("$y", Person('$x')))
...
def formula2b():
    ...
    # BEGIN_YOUR_CODE (around 1 line of code expected)
    #return Forall('$x', Exists('$x', Implies(Person('$x'), Forall('$y', Not(Child('$x', '$y'))))))
    return Exists('$x', Forall('$y', Equiv(Person('$x'), Not(Child('$x', '$y')))))
    #return Exists('$x', Implies(Person('$x'), Forall('$y', Child('$x', '$y'))))
    #return Forall('$y', Exists('$x', Not(Child('$x', '$y'))))
    #return Forall('$y', Exists('$x', Not(Child('$x', '$y'))))
    # END_YOUR_CODE
...
def formula2d():
    ...
    # BEGIN_YOUR_CODE (around 5 lines of code expected)
    #return Forall('$x', Forall('$y', Forall('$z', Equiv(Grandmother('$x', '$z'),
    #    AndList([Female('$z'), Parent('$x', '$y'), Parent('$y', '$z')])))))
    #return Forall('$x', Forall('$y', Forall('$z', Implies(Grandmother('$x', '$z'),
    #    AndList([Female('$z'), Parent('$y', '$z'),Parent('$x', '$y') ])))))
    #return Forall('$x', Forall('$y', Forall('$z', 
    #        AndList([Female('$z'), Parent('$y', '$z'),Parent('$x', '$y') ]),
    #    )))
    #return Forall('$x', Forall('$y', Forall('$z', Equiv(Not(Grandmother('$x', '$z')),
    #        Not(AndList([Female('$z'), Parent('$y', '$z'),Parent('$x', '$y') ])),
    #    ))))
...
def liar():
    ...
    formulas.append(Exists('$x', And(TellTruth('$x'), Forall('$y',
        Implies(Equiv(TellTruth('$x'), TellTruth('$y')),
            Equals('$x', '$y'))))))
    formulas.append(Exists('$x', And(CrashedServer('$x'), Forall('$y',
        Implies(Equiv(CrashedServer('$x'), CrashedServer('$y')),
            Equals('$x', '$y'))))))
    # END_YOUR_CODE
...
def ints():
    ...
    formulas.append(Forall('$y', Exists('$x', 
        And(Successor('$y', '$x'), Forall('$z',
            Implies(Successor('$y', '$z'), And(Equals('$x', '$z'),
                Not(Equals('$y', '$z'))))
            )))))
    ...
    formulas.append(Forall('$x', Forall('$y',
        Implies(
        And(Successor('$x', '$y'),Even('$x')),
        Odd('$y')
            ))))
    formulas.append(Forall('$x', Forall('$y',
        Implies(
        And(Successor('$x', '$y'),Odd('$x')),
        Even('$y')
            ))))
...
def createNLIGrammar():
    ...
    rules.append(GrammarRule('$Clause', ['every', '$Noun', 'has', 'a', '$Noun'],
        lambda args: Forall('$x', Exists('$y', Has(Atom(args[0].title(), '$x'), Atom(args[1].title(), '$y'))))))
    rules.append(GrammarRule('$Clause', ['no', '$Noun', 'has', 'a', '$Noun'],
        lambda args: Not(Exists('$x', Exists('$y', Has(Atom(args[0].title(), '$x'), Atom(args[1].title(), '$y')))))))
    rules.append(GrammarRule('$Clause', ['if', 'a', '$Noun', 'has', 'a', '$Noun', 'then', 'it', 'has', 'a', '$Noun'],
        lambda args: Implies(Exists('$x', Has(Atom(args[0].title(), '$x'), Atom(args[1].title(), '$x'))), Has(Atom(args[0].title(), '$x'), Atom(args[2].title(), '$x')))))
    # END_YOUR_CODE
    return rules

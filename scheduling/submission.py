...
def create_nqueens_csp(n = 8):
    ...
    for i in range(n):
        for j in range(i+1, n):
            # the same row
            csp.add_binary_factor(variables[i], variables[j], lambda x, y: x!=y)
            csp.add_binary_factor(variables[i], variables[j], lambda x, y: x!=y+(j-i))
            csp.add_binary_factor(variables[i], variables[j], lambda x, y: x!=y-(j-i))
    # END_YOUR_CODE
    return csp
...
class BacktrackingSearch():
    ...
    def get_unassigned_variable(self, assignment):
        ...
        if not self.mcv:
            ...
            # BEGIN_YOUR_CODE (around 10 lines of code expected)
            varConstrains = [[sum( self.get_delta_weight(assignment, var,v) for v in self.domains[var] ), var] \
                        for var in self.csp.variables if var not in assignment ]
            mcv = min(varConstrains, key = lambda k: k[0])
            return mcv[1]
            # END_YOUR_CODE
    ...
    def arc_consistency_check(self, var):
        ...
        while len(queue) != 0:
            x = queue.pop(0)
            for var2 in self.csp.get_neighbor_vars(x):
                domainChanged = False
                domainNew =  []
                for val2 in self.domains[var2]:
                    unaryConsistent = True
                    binaryConsistent = False
                    if self.csp.unaryFactors[var2] is not None:
                        unaryConsistent = bool(self.csp.unaryFactors[var2].get(val2, 1) != 0)
                    for val in self.domains[x]:
                        if self.csp.binaryFactors[x][var2][val][val2] != 0:
                            binaryConsistent = True
                            break
                    if ( unaryConsistent and binaryConsistent):
                        domainNew.append(val2)
                    else:
                        domainChanged = True
                if domainChanged:
                    queue.append(var2)
                    self.domains[var2] = domainNew
        # END_YOUR_CODE
...
def get_sum_variable(csp, name, variables, maxSum):
    ...
    for i, Xi in enumerate(variables):
        Ai = ('sum', name, Xi)
        if i == 0:
            domains = [(0,k,k) for k in csp.values[Xi]]
        else:
            domains = [(a[2],b,a[2]+b) for a in csp.values[Aiprev] for b in csp.values[Xi] \
                    if a[2]+b <= maxSum]
            domains = list(set(domains))
        csp.add_variable(Ai, domains)
        csp.add_binary_factor(Xi, Ai, valueisSecond)
        if i > 0:
            csp.add_binary_factor(Aiprev, Ai, aisFirst)
        Aiprev = Ai
    
    newDomains = list(set([d[2] for d in domains]))
    csp.add_variable(result, newDomains)
    csp.add_binary_factor(result, Ai, aisThird)
    return result
    # END_YOUR_CODE
...
class SchedulingCSPConstructor():
    ...
    def add_quarter_constraints(self, csp):
        ...
        for req in self.profile.requests:
            ...
            #totalOr = util.get_or_variable(csp, ('totalquarter', req), orVars, False)
            #csp.add_unary_factor(totalOr, lambda v: not v )
        # EN#D_YOUR_CODE
    ...
    def add_unit_constraints(self, csp):
        ...
        for quarter in self.profile.quarters:
            ...
            for req in self.profile.requests:
                for cid in req.cids:
                    varname = (cid, quarter)
                    csp.add_variable(varname, [0] + range(self.bulletin.courses[cid].minUnits, \
                            self.bulletin.courses[cid].maxUnits+1) )
                    sumMax += self.bulletin.courses[cid].maxUnits
                    def courseEq(c, unit):
                        if ( c == cid):
                            return unit != 0
                        else:
                            return unit == 0
                    csp.add_binary_factor((req,quarter), varname, courseEq)
                    quarterVars.append(varname)
            quarterSum = get_sum_variable(csp, 'unit_sum_' + quarter, quarterVars, self.profile.maxUnits)
            csp.add_unary_factor(quarterSum, lambda a: a <= self.profile.maxUnits and a >= self.profile.minUnits)
            print csp.variables
            a = 1/0
        # END_YOUR_CODE

...
class ExactInference(object):
    ...
    def observe(self, agentX, agentY, observedDist):
        ...
        for row in range(self.belief.getNumRows()):
            for col in range(self.belief.getNumCols()):
                y = util.rowToY(row)
                x = util.colToX(col)
                dist = ((x-agentX)**2 + (y-agentY)**2)**(0.5)
                newBelieve = util.pdf(dist, Const.SONAR_STD, observedDist)
                nb = self.belief.getProb(row,col) * newBelieve
                self.belief.setProb(row,col, nb)
    ...
    def elapseTime(self):
        ...
        for (oldTile, newTile) in self.transProb:
            ...
            oldb = self.belief.getProb(oldrow, oldcol)
            prob = oldb * prob
            if newTile not in record:
                record[newTile] = prob 
            else:
                record[newTile] += prob
        for row in range(self.belief.getNumRows()):
            for col in range(self.belief.getNumCols()):
                if (row, col) in record:
                    self.belief.setProb(row, col, record[(row,col)])
                else:
...
class ParticleFilter(object):
    ...
    def observe(self, agentX, agentY, observedDist):
        ...
        particleWeight = {}
        for tile in self.particles:
            # update weight for all particles
            row, col = tile
            y = util.rowToY(row)
            x = util.colToX(col)
            dist = ((x-agentX)**2 + (y-agentY)**2)**(0.5)
            newBelieve = util.pdf(dist, Const.SONAR_STD, observedDist)
            nb = self.belief.getProb(row,col) * newBelieve
            particleWeight[tile] = nb
        # resample
        particles = collections.Counter()
        for i in range(self.NUM_PARTICLES):
            tile = util.weightedRandomChoice(particleWeight)
            particles[tile] += 1
    ...
    def elapseTime(self):
        ...
        for oldTile in self.particles:
            for i in range(self.particles[oldTile]):
